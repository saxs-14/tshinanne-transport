...
export default function Reports(){
 ...
 const truckTotals=useMemo(()=>trucks.map(t=>{const ds=filtered.deliveries.filter(d=>d.truckId===t.id);const es=filtered.expenses.filter(e=>e.truckId===t.id);const revenue=ds.reduce((s,d)=>s+Math.max(0,Number(d.price)||0),0);const costs=es.reduce((s,e)=>s+Math.max(0,Number(e.amount)||0),0);return{...t,deliveries:ds.length,revenue,costs,profit:revenue-costs}}).sort((a,b)=>b.revenue-a.revenue),[trucks,filtered])
