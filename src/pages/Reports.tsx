...
export default function Reports(){
 ...
 return <section className="page-section">
  ...
  <div className="finance-stats">
   ...
   <div className="finance-stat"><div><span>Total operating costs</span><strong>{money(totals.expenseTotal)}</strong><small>Includes recorded fuel</small></div><BarChart3 size={22}/></div>
   <div className="finance-stat highlight"><div><span>Estimated profit</span><strong>{money(totals.profit)}</strong><small>{filtered.deliveries.length} deliveries · fuel included</small></div><Package size={22}/></div>
  </div>
  <div className="report-grid">
   <div className="page-card"><p className="eyebrow">Expenses</p><h3>By category</h3>{categoryTotals.length===0?<p className="muted">No manual expenses in this period.</p>:<div className="bar-list">{categoryTotals.map(([name,value])=><div className="bar-row" key={name}><div><strong>{name}</strong><span>{money(value)}</span></div><div className="bar-track"><i style={{width:`${Math.max(5,(value/maxCategory)*100)}%`}}/></div></div>)}</div>}</div>
   <div className="page-card"><p className="eyebrow">Fuel</p><h3>Fuel analytics</h3><div className="report-number"><Fuel size={22}/><strong>{totals.fuelLitres.toLocaleString('en-ZA',{maximumFractionDigits:1})} L</strong></div><p className="muted">{money(totals.fuelCost)} recorded fuel cost in this period. Fuel is included in operating costs above.</p></div>
  </div>
