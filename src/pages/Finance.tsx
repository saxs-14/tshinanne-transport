...
type FuelRecord = { id:string; truckId?:string; amount?:number; date:string }
type Truck = { id:string; registrationNumber:string }
const categories=['Maintenance','Tyres','Repairs','Driver costs','Other']

export default function Finance(){
 const [expenses,setExpenses]=useState<Expense[]>([]),[deliveries,setDeliveries]=useState<Delivery[]>([]),[fuel,setFuel]=useState<FuelRecord[]>([]),[trucks,setTrucks]=useState<Truck[]>([])
 const [isOwner,setIsOwner]=useState<boolean|null>(null),[open,setOpen]=useState(false),[error,setError]=useState('')
 const [form,setForm]=useState({category:'Maintenance',amount:0,description:'',date:new Date().toISOString().slice(0,10),truckId:''})
 ...
 return <section className="page-section">
  <div className="section-heading"><div><p className="eyebrow">Money & reporting</p><h2>Finance</h2><p className="muted">Track revenue, payments, operating costs and estimated profit.</p></div><button className="primary-button compact" onClick={()=>{setError('');setOpen(true)}}><Plus size={17}/> Add expense</button></div>
  ...
  <div className="finance-stats">
   ...
   <div className="finance-stat"><div><span>Total operating costs</span><strong>{money(totals.expensesTotal)}</strong><small>Includes fuel from Fuel records</small></div><TrendingDown size={22}/></div>
   <div className="finance-stat highlight"><div><span>Estimated profit</span><strong>{money(totals.profit)}</strong><small>Revenue minus operating costs</small></div><BarChart3 size={22}/></div>
  </div>
  <div className="page-card"><p className="eyebrow">Fuel</p><h3>Recorded fuel costs</h3><div className="report-number"><Fuel size={22}/><strong>{money(totals.fuelCost)}</strong></div><p className="muted">Fuel entered in the Fuel section is automatically included in total operating costs. Do not record the same fuel purchase again as a manual expense.</p></div>
  ...
  {open&&<div className="modal-backdrop"><form className="modal" onSubmit={saveExpense}><div className="modal-heading"><div><p className="eyebrow">Business cost</p><h3>Add expense</h3></div><button type="button" className="close-button" onClick={()=>setOpen(false)} aria-label="Close"><span>×</span></button></div>
   ...
   <label>Description<input value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="e.g. Truck repair" required/></label>
