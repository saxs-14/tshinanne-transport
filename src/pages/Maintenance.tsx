...
export default function Maintenance(){
 const[trucks,setTrucks]=useState<Truck[]>([]),[records,setRecords]=useState<MaintenanceRecord[]>([])
 ...
 function startNew(){setEditing(null);setForm({...empty,truckId:trucks[0]?.id??'',odometer:trucks.find(t=>t.id===trucks[0]?.id)?.currentOdometer??0});setError('');setOpen(true)}
 ...
 return <section className="page-section">
  <div className="section-heading"><div><p className="eyebrow">Fleet care</p><h2>Maintenance</h2><p className="muted">Track services, repairs, tyres and the next time each truck needs attention.</p></div><button className="primary-button compact" onClick={startNew}><Plus size={17}/> Add record</button></div>
  ...
  {open&&<div className="modal-backdrop"><form className="modal" onSubmit={save}><div className="modal-heading"><div><p className="eyebrow">Fleet record</p><h3>{editing?'Edit maintenance':'Add maintenance'}</h3></div><button type="button" className="close-button" onClick={()=>setOpen(false)}><X/></button></div>
   <label>Truck<select value={form.truckId} onChange={e=>setForm({...form,truckId:e.target.value})}>{trucks.map(t=><option key={t.id} value={t.id}>{t.registrationNumber} — {t.make} {t.model}</option>)}</select></label>
